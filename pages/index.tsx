...
function AuthFullScreen({
  visible, onClose, onAuthed, sessionId
}: { visible: boolean; onClose: () => void; onAuthed: (email: string, displayName?: string) => void; sessionId: string }) {
  ...
  const attachSession = async (emailArg: string) => {
    try {
      await fetch("/api/portal/docs", {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ action:"attach-session", email: emailArg, sessionId })
      });
    } catch {}
  };
...
/* =============== FULL-SCREEN PORTAL =============== */
type DocRow = { id: string; file_name: string; file_url: string; uploaded_at: string };

function PortalFullScreen({
  visible, onClose, displayName, loggedEmail, sessionId, bump
}: { visible: boolean; onClose: () => void; displayName?: string; loggedEmail?: string; sessionId: string; bump: number }) {
  ...
  // Load documents whenever portal opens, email changes, or an upload happens (bump)
  const loadDocs = async () => {
    if (!loggedEmail) return setDocs([]);
    try {
      const r = await fetch(`/api/portal/docs?email=${encodeURIComponent(loggedEmail)}`);
      const j = await r.json();
      if (j?.ok) setDocs(j.documents || []);
    } catch {}
  };
  useEffect(()=>{ if (visible) loadDocs(); }, [visible, loggedEmail, bump]);
  ...
  const saveProfile = async () => {
    try {
      ...
      const j = await r.json();
      setDocNotice(j?.ok ? "Saved." : (j?.error || "Save failed."));
      setTimeout(()=>setDocNotice(""), 2000);
    } catch {
      setDocNotice("Save failed (network).");
      setTimeout(()=>setDocNotice(""), 2000);
    }
  };

  // Upload from portal Docs tab
  const handlePortalUpload = async (file: File) => {
    try {
      ...
      const j = await r.json();
      if (j?.ok) {
        setDocNotice("Uploaded.");
        await loadDocs();
      } else {
        setDocNotice(j?.error || "Upload failed.");
      }
    } catch {
      setDocNotice("Upload failed (network).");
    } finally {
      setTimeout(()=>setDocNotice(""), 2000);
    }
  };
            ...
            {tab==="debts" && (
              <div style={{display:"grid", gap:12}}>
                ...
                <div style={{marginTop:6, color:"#6b7280", fontSize:12}}>
                  You can add more later. Upload creditor letters in the <strong>Documents</strong> tab or via the chat uploader (📎).
                </div>
              </div>
            )}
                ...
                <div style={{display:"flex", alignItems:"center", justifyContent:"space-between"}}>
                  ...
                  <label style={{display:"inline-flex", alignItems:"center", gap:8, cursor:"pointer"}}>
                    <input
                      type="file"
                      style={{ display:"none" }}
                      onChange={e=>{ const f=e.target.files?.[0]; if (f) handlePortalUpload(f); (e.target as HTMLInputElement).value=""; }}
                    />
                    <span style={{padding:"8px 12px", borderRadius:8, border:"1px solid #d1d5db", background:"#fff"}}>Upload more</span>
                  </label>
                </div>
                    ...
                    {docs.map(d=>(
                      <a key={d.id} href={d.file_url} target="_blank" rel="noreferrer"
                         style={{display:"flex", justifyContent:"space-between", alignItems:"center",
                                 padding:"10px 12px", border:"1px solid #e5e7eb", borderRadius:8, textDecoration:"none", color:"#111827"}}>
...
/* =============== Chat UI (with timestamps + upload ping) =============== */
export default function Home() {
  ...
  const styles: { [k: string]: React.CSSProperties } = {
    ...
    meta: { display:"block", fontSize:11, opacity:.7, marginTop:4 },
    attach: { marginTop:8 },
      ...
      <main style={styles.frame}>
        <div style={styles.card}>
              ...
              return (
                <div key={i} style={{...styles.row, ...(isUser?styles.rowUser:{})}}>
                  ...
                  <div style={{...styles.bubble, ...(isUser?styles.bubbleUser:styles.bubbleBot)}}>
                    ...
                    {m.at && <span style={styles.meta}>{m.at}</span>}
                  </div>
                </div>
              );
            })}
          ...
          <div style={styles.footer}>
            <button type="button" title="Upload a file" style={styles.btn} onClick={()=>fileRef.current?.click()}>📎</button>
            <input ref={fileRef} type="file" style={{display:"none"}} onChange={e=>{ const f=e.target.files?.[0]; if (f) onChatUpload(f); (e.target as HTMLInputElement).value=""; }} />
            <input
